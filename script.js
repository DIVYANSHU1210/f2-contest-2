...
function addAll(studentData){
    for(let i=0; i<studentData.length; i++){
        let trow = document.createElement("tr");
        trow.innerHTML=`
            <td>${studentData[i].id}</td>
            <td><img src = ${studentData[i].img_src}> <span>${studentData[i].first_name} ${studentData[i].last_name}</span></td>
            <td>${studentData[i].gender}</td>
            <td>${studentData[i].class}</td>
            <td>${studentData[i].marks}</td>
            <td>${studentData[i].passing===true?"passing":"failed"}</td>
...
function sortByPassing(){
    let passedStudents = studentData.filter((a)=>{
        return a.passing === true;
    })

    tbody.innerHTML = "";

    addAll(passedStudents);
}
...
function sortByGender(){
    ...
    let maleStudents = studentData.filter((a)=>{
        return a.gender === "Male";
    })

    let femaleStudents = studentData.filter((a)=>{
        ...
        for(let i=0; i<maleStudents.length; i++){
            let trow = document.createElement("tr");
            trow.innerHTML=`
                <td>${maleStudents[i].id}</td>
                <td><img src = ${maleStudents[i].img_src}> <span>${maleStudents[i].first_name} ${maleStudents[i].last_name}</span></td>
                <td>${maleStudents[i].gender}</td>
                <td>${maleStudents[i].class}</td>
                <td>${maleStudents[i].marks}</td>
                <td>${maleStudents[i].passing===true?"passing":"failed"}</td>
                <td>${maleStudents[i].email}</td>
            `
            t1.appendChild(trow);
        }
        ...
        for(let i=0; i<femaleStudents.length; i++){
            let trow = document.createElement("tr");
            trow.innerHTML=`
                <td>${femaleStudents[i].id}</td>
                <td><img src = ${femaleStudents[i].img_src}> <span>${femaleStudents[i].first_name} ${femaleStudents[i].last_name}</span></td>
                <td>${femaleStudents[i].gender}</td>
                <td>${femaleStudents[i].class}</td>
                <td>${femaleStudents[i].marks}</td>
                <td>${femaleStudents[i].passing===true?"passing":"failed"}</td>
                <td>${femaleStudents[i].email}</td>
            `
            t2.appendChild(trow);
        }
...
function search(event){
    ...
    const searchResults = studentData.filter((a) => {
        if(a.first_name.match(pattern)){
            return true;
        }
        else if(a.last_name.match(pattern)){
            return true;
        }
        else if(a.email.match(pattern)){
            return true;
        }
        return false;
    });
